...
beta0 = st.sidebar.slider("β L0", min_value=0.0000, max_value=0.2, value=0.0005, step=0.0001, format="%.4f")
beta1 = st.sidebar.slider("β L1", min_value=0.0000, max_value=0.2, value=0.0005, step=0.0001, format="%.4f")
beta2 = st.sidebar.slider("β L2", min_value=0.0000, max_value=0.2, value=0.0005, step=0.0001, format="%.4f")
beta_vec = [beta0, beta1, beta2]

st.sidebar.markdown("---")
st.sidebar.markdown("### Infectious Flow Rate Multipliers (0.0001×–5×)")
# Existing
m_EI   = st.sidebar.slider("E→I ×",    0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_IP   = st.sidebar.slider("IP→IS ×",  0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_ISR  = st.sidebar.slider("IS→R ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_IAR  = st.sidebar.slider("IA→R ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")
# New: hospital flow
m_ISH  = st.sidebar.slider("IS→H ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_HR   = st.sidebar.slider("H→R ×",    0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_HD   = st.sidebar.slider("H→D ×",    0.0000, 5.0, 1.0, 0.01, format="%.4f")

st.sidebar.markdown("---")
st.sidebar.markdown("### Split / Infectiousness Multipliers")
# E_to_IA_prop is a proportion; scale then clip [0,1]
m_EIAp = st.sidebar.slider("E→IA proportion × (clip 0–1)", 0.0000, 5.0, 1.0, 0.01, format="%.4f")
# Relative infectiousness (>0)
m_IPinf = st.sidebar.slider("IP relative infectiousness ×", 0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_IAinf = st.sidebar.slider("IA relative infectiousness ×", 0.0000, 5.0, 1.0, 0.01, format="%.4f")

st.sidebar.markdown("---")
st.sidebar.markdown("### Reinfection / Immunity Multipliers")
m_RS   = st.sidebar.slider("R→S × (reinfection)",             0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_wane_inf = st.sidebar.slider("inf_induced_immune_wane ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_wane_vax = st.sidebar.slider("vax_induced_immune_wane ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")

st.sidebar.markdown("#### Risk Reduction Multipliers (clip 0–1)")
m_inf_inf   = st.sidebar.slider("inf_induced_inf_risk_reduce ×",    0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_vax_inf   = st.sidebar.slider("vax_induced_inf_risk_reduce ×",    0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_inf_hosp  = st.sidebar.slider("inf_induced_hosp_risk_reduce ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_inf_death = st.sidebar.slider("inf_induced_death_risk_reduce ×",  0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_vax_hosp  = st.sidebar.slider("vax_induced_hosp_risk_reduce ×",   0.0000, 5.0, 1.0, 0.01, format="%.4f")
m_vax_death = st.sidebar.slider("vax_induced_death_risk_reduce ×",  0.0000, 5.0, 1.0, 0.01, format="%.4f")
